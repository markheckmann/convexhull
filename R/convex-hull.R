...
# convex hulls with alpha-% of the points by removing the most outlying points.
# Function removes proportion of points that is most distant from the centroid.
#
# xy :    xy.coords object
#
...
#' 
#' @param x,y  The x and y coordinates of a set of points. Alternatively, a
#'   single argument x can be provided.
#' @param alpha Proportion of points most distant from centroid to be removed
#'   before creating the (peeled) convex hull.
#' @param peel Remove hull points? (useful to remove outliers, default
#'   \code{FALSE})
#' @param smooth Integer (\code{1} to \code{3}) indicating the method for
#'   creating splines around the hull. \code{0} (default) is no smooting.
#' @param shape Shape parameter used for \code{smooth=1} (see \code{xspline} for
#'   details).
#' @param draw Draw the hull polygon (default \code{TRUE}).
#' @param ... additional parameters passed to \code{polygon} to draw the hull.
#' @return A list as given by \code{\link{xy.coords}} containing the hull points
                        ...
                        smooth=0, shape=1, draw=TRUE, ...)
{
  ...
  # less than 3 points cannot be interpolated: warn user and omit drawing
  if (length(xy$x) < 3) {
    warning("convex_hull: At least three points needed for smoothing. No hull drawn.", call. = FALSE)
    return(NULL)
  }
